    ...
    <div className="container">
      ...
      >
        <div style={{
          ...
          <h4 style={{marginBottom:25,marginTop:25,fontSize:18,fontWeight:500}}>{actionId===1?"确定要删除此支付账户吗？":"确定要删除这个模型吗？"}</h4>
          {(()=>{
          if(actionId===1){
            return <p>模型<span style={{color:"#2C68FF"}}> 哔哩哔哩</span> 一共包含了 15 个商品，包含商品的模型不允许被删除，请更改关联商品的模型之后重试。</p>
          }
            return <p>删除的模型不可被找回，请确认。</p>
          })()}
  ...
  return (
    <div style={styles.main}>
      <div style={{
        ...
        <div>
          <Input placeholder="请输入模型名称" size="small" style={{width:120,fontSize:12}}/>
          <Button icon={
            <img src={good9} alt="" style={{width:10,marginRight:5,marginBottom:1}} />
          }
          type = "primary"
          size = "small"
            style = { { fontSize: 12,height:20,marginLeft:20} } > 搜索模型 < /Button>
        </div>
        <Button icon={
        ...
        type = "primary"
        size = "small"
          style = { { fontSize: 12,height:20 } } > 新建模型 < /Button>
      </div>
      <div style={{
        display:'flex',
        alignItems:'center',
