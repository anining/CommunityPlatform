...
function RGroup ({ tagsGroup, setTagsGroup }) {
  ...
  function handleInputConfirm (index, id) {
    if (inputValue && tagsGroup[index].tags.indexOf(inputValue) === -1) {
      tagsApi('add', undefined, { name: inputValue, tag_group_id: id }).then(r => {
        ...
        r.error && setTagsGroup([...tagsGroup][index].tags.filter(item => item.name !== inputValue))
      })
      const tags = [...tagsGroup]
      tags[index].tags.push(inputValue)
      setTagsGroup(tags);
      setInputVisible(false)
      setInputValue(undefined)
    }
  ...
  tagsGroup.forEach((item, index) => {
    ...
    if (inputVisible && value === index) {
      ...
      items.push(
        <Tag onClick={()=>{
          ...
          // saveInputRef.focus()
          setValue(index)
        }} className={cs.tAdd} key={Math.random()}>
          <img src={edit1} alt="" />
          <div>添加标签</div>
        </Tag>
      )
    ...
    views.push(
      <div key={index} className={cs.tItemView}>
